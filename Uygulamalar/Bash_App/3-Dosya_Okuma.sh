...
if [[ -f "deneme.txt" ]]; then
	...
	while IFS= read -r okudu_sey; 
	do
	echo "Okunan Satır:$okudu_sey"
	done < "deneme.txt"
else
...
# `IFS`, yani **Internal Field Separator**, Bash kabuğunda veri ayrıştırmak için kullanılan **özel bir değişkendir**. Genellikle `read`, `for`, `set`, ve `awk` gibi komutlarla kullanılırken satır veya kelime bazlı veri işlemede alanların nasıl bölüneceğini belirler.
